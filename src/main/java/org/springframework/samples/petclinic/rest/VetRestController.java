...
public class VetRestController {
    ...
    @RequestMapping(value = "/{vetId}/petsVisited", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<Collection<Pet>> getPetsVisited(@PathVariable("vetId") int vetId){
        Vet vet = this.clinicService.findVetById(vetId);
        if(vet == null){
            return new ResponseEntity<Collection<Pet>>(HttpStatus.NOT_FOUND);
        }

        Collection<Pet> petsVisited = vet.getPets();
